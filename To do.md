...
- Edit discussion of vectors at beginning of chapter to discuss the following... There is circularity between intuitive notions of "point" and "vector". We can resolve this circularity on a formal level by defining each of "point" and "vector" in more primitive terms. Though, there is a circularity between the intuitive ideas of "point" and "vector" that will always persist.
  - Suppose we attempt to make "vector" our primitive concept on top of which we define "point". We may attempt to define a "point" to be a sum of vectors cooresponding to the directions of the coordinate axes. But a geometric understanding what a "sum" of vectors is requires a notion of point (a sum of vectors is understood as using the first vector in the sum to arrive at a point, and then placing the "tail" of the other vector in the sum on that point). So we see that, intuitively, the notion of "vector" depends on the notion of "point".
  - Suppose we attempt to make "point" our primitive concept on top of which we define "vector". We may attempt to define a "vector" to be a difference of points. But a geometric understanding of what a difference of points requires a notion of "vector" (the difference of points $\mathbf{p}\_2 - \mathbf{p}\_1$ is understood as being the difference between the vectors $(\mathbf{p}\_2 - \mathbf{q})$ and $(\mathbf{p}\_1 - \mathbf{q})$, where $\mathbf{q}$ is any point). So we see that, intuitively, the notion of "point" depends on the notion of "vector".
